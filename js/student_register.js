  function registerUser(event) {
    ...
    if (!/^[A-Za-z]+(\s[A-Za-z]+)+$/.test(fullname)) {
      alert("🧾 Please enter your full name with at least first and last name (e.g., Deep Kumar).");
      return;
   }
    ...
    if (altmobile && !/^[6-9]\d{9}$/.test(altmobile)) {
      alert("📞 Please enter a valid 10-digit Indian alternate number (starting with 6-9)!");
      return;
    }
    ...
    if (!passwordPattern.test(password)) {
      alert("🔐 Password must be at least 6 characters long and include a number and special character!");
      return;
    }
